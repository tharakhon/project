    ...
    useEffect(() => {
        Axios.get(`http://localhost:5000/user/${username}/${bank_name}`)
            .then((response) => {
                ...
                setRankImage(response.data[0].rank_image);
                setRankName(response.data[0].rank_name);
            })
            .catch((error) => {
    ...
    return (
        <div>
            ...
            >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', width: '48ch' }}>
                    <TextField
                        id="outlined-basic"
                        variant="outlined"
                        disabled
                        InputProps={{
                            startAdornment: (
                                <InputAdornment position="start">
                                    <div style={{ width: '52px', height: '52px', backgroundImage: `url(http://localhost:5000/image/${rankImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}></div>
                                </InputAdornment>
                            )
                        }}
                        value={rankName}
